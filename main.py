...
async def answer13(message: types.Message, state: FSMContext):
    if message.text == "/back - назад":
        ...
        await add_answer(message, state, 0, 2, "Введите четвертый вариант ответа")
        await NewTest.next()

@dp.message_handler(state=NewTest.answer14)
async def answer14(message: types.Message, state: FSMContext):
    if message.text == "/back - назад":
        await message.answer('Отмена', reply_markup=menu_admin if db.user_is_admin(message.from_user.id) else menu)
        await state.finish()
    else:
        await add_answer(message, state, 0, 3, "Введите номер правильного ответа (1-4)")
        await NewTest.next()
...
async def answer23(message: types.Message, state: FSMContext):
    if message.text == "/back - назад":
        ...
        await add_answer(message, state, 1, 2, "Введите четвертый вариант ответа")
        await NewTest.next()

@dp.message_handler(state=NewTest.answer24)
async def answer24(message: types.Message, state: FSMContext):
    if message.text == "/back - назад":
        await message.answer('Отмена', reply_markup=menu_admin if db.user_is_admin(message.from_user.id) else menu)
        await state.finish()
    else:
        await add_answer(message, state, 1, 3, "Введите номер правильного ответа (1-4)")
        await NewTest.next()
...
async def answer33(message: types.Message, state: FSMContext):
    if message.text == "/back - назад":
        ...
        await add_answer(message, state, 2, 2, "Введите четвертый вариант ответа")
        await NewTest.next()

@dp.message_handler(state=NewTest.answer34)
async def answer34(message: types.Message, state: FSMContext):
    if message.text == "/back - назад":
        await message.answer('Отмена', reply_markup=menu_admin if db.user_is_admin(message.from_user.id) else menu)
        await state.finish()
    else:
        await add_answer(message, state, 2, 3, "Введите номер правильного ответа (1-4)")
        await NewTest.next()
    ...
    if db.user_is_admin(message.from_user.id):
        ...
        for d in docs:
            ...
            for s in solved_test:
                ...
                sheet.write(line, 5, str(user[4]), styleLeftData)
...
async def check_questions(message: types.Message, state: FSMContext, question, index_question):
    ...
    try:
        ...
        if 0 < answer < 5:
            ...
            await message.answer("Введите корректный ответ (1-4)")
            await question.set()
    except ValueError:
        await message.answer("Введите корректный ответ (1-4)")
        await question.set()
...
async def test_results(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        answer = int(message.text)
        if 0 < answer < 5:
            if db.check_answer(data.get("id_question")[2], answer):
                async with state.proxy() as data:
                    data["score"] += 1
